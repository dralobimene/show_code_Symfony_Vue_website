...
// fichier: src/views/view12_adm_delete_comment.vue
// controller associé: src/Controller/DeleteCommentController.php
...
fetch(`${apiBase}/adm/delete_comment`, {
  headers: {
  ...
  .then((response) => {
    if (!response.ok) {
      throw new Error("Error 01 from view12_adm_delete_comment, network response was not ok");
    }
  ...
  .catch((error) => {
    console.log("Error from view12_adm_delete_comment: ", error);
  });
  ...
  <main>
    ...
    <section class = "section01_connected_user_informations">
      ...
      <div v-if = "errorOccurred" class = "error-message">
            Message from view12: An error occurred: {{ errorMessage }}
      </div>
